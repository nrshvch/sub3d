...
**Diagnostics.** `gfx.canvas.accelerated.debug` paints a 16x16 green square at the top-right of every _accelerated_ canvas each frame; a software canvas never gets one. The layer canvases are never in the DOM, so read it with `getImageData(w - 8, 4, 1, 1)` on `viewport.layers` / `.shadeLayers` / `.fogLayers` and check for exact `(0, 255, 0)` — the examples expose `window.viewport` for this. Do it on demand, not per frame: `getImageData` is itself a readback and counts in the ratio above. The debug panel's **Weld Verts / Points** and **Expand Ratio** rows come from `STATS_WELD_VERTS` / `STATS_WELD_POINTS` in `weld.js`; the ratio is points emitted over boundary vertices, 1.0 with no seam repair and 3.0 when every boundary edge is owned.
