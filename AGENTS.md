...
**Coplanar merging is not implemented, and single-slot merging is not the way to get it.** Merging the two triangles of a face into one polygon removes the shared diagonal entirely — cheaper and better looking, since one affine map has no crease where two disagree. But measured on the boxes scene, only **8%** of textured faces arrive with their coplanar partner immediately before them: the depth sort scatters them exactly as it scatters same-colour flat faces. A single pending slot would therefore merge ~4% of fills. Doing it properly needs the multi-slot welder generalised to carry a per-slot affine and emit its ring in texture space, keyed on mesh + texture + plane — the same conclusion the flat pass reached when one slot gave 2 merges in 3900 faces.
...
The cost of that split is paid in the shade pass, and is worth knowing before chasing draw calls there. `ALBEDO_FLAT` and `TEXTURE` share one shade function and one weld state, but `last` is keyed on the *shaderKey*, so every texture↔colour transition flushes the shade welder even though nothing about the shading changed. Measured on the boxes example (500 meshes, about half textured, same geometry either way) that is 1561 shade draw calls against 1301 when every mesh carries one key — roughly 20%. Keying `last` on the shade function's identity rather than the fill shader's would recover it.
