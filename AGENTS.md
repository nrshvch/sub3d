...
5. **Three draw passes**, each onto its own offscreen canvas per layer (`viewport.layers[i]` / `.shadeLayers[i]` / `.fogLayers[i]`):
   ...
   - **Fog** (`fogPass`, `src/shaders/flatFog/fog.js`) — its own sort (`fogSort`, mirroring `radixSort`'s key order with a fog-bucket tie-break) and its own offscreen layer, composited back with `difference`/`multiply`/`lighter` blend steps depending on fog type. The `difference` inversion runs on a fourth scratch canvas, `.fogCompositeLayers[i]`, and is the only blend that canvas ever does: Firefox's accelerated canvas has no `difference` blend, so whichever canvas draws it falls back to software every frame and demotes by frame 10. The tint is folded in ahead of it on the fog buffer as `screen(keep, 1 - c) = keep * c + (1 - c)`, which is accelerated, so the demoted canvas does the one full-screen blend nothing else can. `prepareFog` admits only the flat fill keys (`ALBEDO_FLAT`, `TEXTURE`) and marks everything else skipped — a face it skips keeps the fog buffer's black background under it, which reads as _fully fogged_, so a new flat-family shaderKey has to be added to that filter or its meshes will render solid `fogColor`.

     **The pass decides for itself whether to run, and `fogPass` is the only way in.** It is a post-process over what fill and shade already drew, so nothing about that decision reaches them - which is why `fillTriangles` and `shadeTriangles` clear their own buffers unconditionally rather than relying on the fog composite to erase last frame. `prepareFog` walks the layer's faces once ahead of the sort, computes every fog amount exactly once into the pass's own module-private scratch for the sort and the raster to reuse, and returns three findings: whether anything is fogged at all; the union screen bounds of every not-fully-fogged face; and a per-face "do not draw" verdict. Nothing fogged means the buffer would come out uniformly white and composite back as the identity, so the whole pass is dropped - sort, raster and all five full-screen composite steps. The mirror case is dropped a different way: when _every_ face is fully fogged the buffer would be black edge to edge, and `fill * 0 + fogColor` is flat `fogColor` everywhere including the sky - so the layer is filled with it directly and the sort, the raster and all five composite steps go too. Measured on a 1280x720 frame the composite alone is **0.2ms**, and the result is pixel-identical.
...
- **A matching face normal.** This is the coplanarity requirement and it is _not_ implied by the edge test — two perpendicular box sides share vertex identities as soon as a mesh is position-welded, and edge adjacency alone would fuse them under one nonsense map. A shared edge gives the faces a common point, and a common point plus a common normal is the same plane.
...
Firefox is the engine to measure on, and it must be measured in its **software** state — see "Firefox canvas acceleration" below for why that is the state your users get. A change that looks free in a Chromium-based preview can be a large regression there.

### Firefox canvas acceleration, and why it demotes

Firefox's (v156, Macos(ARM)) accelerated 2D canvas (`DrawTargetWebgl`, `dom/canvas/`) switches a canvas to Skia software rasterisation partway through a session and never switches back. This engine trips it reliably. **That is correct behaviour and should not be fought** — see the verdict at the end. What follows is the mechanism, so it does not get rediagnosed from scratch.

**The demotion rule.** Per frame, `UsageProfile::EndFrame` marks the frame failed when either

    mFallbacks > 0
    (misses + readbacks + layers) > 0.66 * (misses + hits + uncachedDraws + readbacks + layers)

and `RequiresRefresh` then demotes once at least 10 frames have been seen and `failedFrames > 0.3 * frameCount`. `CanvasRenderingContext2D::TryAcceleratedTarget` sets `mAllowAcceleration = false`, which is permanent and per canvas. Frame time is not an input anywhere; neither is path size or complexity. A "cache miss" is counted even for a path the GPU tessellated successfully — it only means the draw was not served from cache. The one term that counts as _good_ alongside hits is an uncached draw, which is a solid-colour axis-aligned rect, i.e. `fillRect`. Nothing a polygon fill does can land there.

**The counters never reset** for the canvas's lifetime, so the threshold has hysteresis: after `F` good frames you need roughly `0.43 * F` failing ones to cross. Reaching a view gradually survives where refreshing straight into it demotes at frame 10, and the same view will demote at wildly different scene sizes depending on how it was approached. Any A/B here has to reload between runs, or the numbers are not comparable.

**What actually governs it is a working set, not a rate.** A cache hit needs the same quantised path at a matching scale; cached tessellations live in one bump-allocated vertex buffer, `gfx.canvas.accelerated.gpu-path-size`, 4MB by default, **shared by every canvas in the process** — the fill, shade and fog layers all compete for it. `mPathVertexOffset` only moves forward, and an allocation that does not fit calls `ClearVertexRanges()`, dropping every cached range everywhere at once. So with `W` = total tessellation bytes of all distinct paths visible at once:

- `W < 4MB` — frame 1 uploads them, every later frame is all hits, the buffer never wraps, the canvas stays accelerated indefinitely.
- `W > 4MB` — the buffer wraps mid-frame, every frame, and most draws miss. Demotes at frame 10.

It is a step function at `W = capacity`, which is why demotion looks like a sharp threshold in window width that moves as the scene changes. Raising `gpu-path-size` moves the threshold proportionally; that is a diagnostic, not a fix.

**Tessellation bytes are not vertex counts.** WGR (`third_party/rust/wpf-gpu-raster`) emits trapezoids spanning many scanlines when it can, and falls back to per-sub-scanline coverage (8 sub-scanlines per pixel row) when it cannot. The fast path needs a full scanline boundary, **no new edge starting within the next pixel row**, and no self-intersection. A merged welded polygon fails all three: 128 vertices scatter edge starts down its height, and the seam expansion crosses itself at every reflex corner. Measured: a convex ring produces no crossings, a staircase strip 5 in 42 points. Consequence, measured on isometric-world: **20k unwelded triangles cost less buffer than 3k merged polygons** — welding off and expansion off survived a grazing angle at 61k emitted points where welding on demoted at 24k.

**Draw-call count is not the driver either.** 60k draw calls of small triangles hold up accelerated at a grazing angle, while a few thousand merged polygons demote in the same scene. What fills the buffer is tessellation output, and on the complex-scan path that scales with the scanlines a path spans and the edges active on each — roughly the pixels it has to cover — not with how many paths there are. That is why a grazing-angle sliver is nearly free despite there being tens of thousands of them, why one tall merged polygon is not, and why shrinking the window is the most reliable way to stop a demotion: fewer scanlines per path, smaller `W`. Optimising for draw calls here is optimising the wrong number.

**The verdict.** Firefox is right. Measured fullscreen at a grazing angle, forcing acceleration to stay on (`gpu-path-size = 8`) gives **12fps against 20fps demoted**. At low path counts acceleration does win — the shade pass measured 3-4ms accelerated against 7-8ms in software — so there is a crossover, and the demotion trigger sits near it. Past it, the per-path cost of tessellating and blitting thousands of unique paths through the remoted backend exceeds what Skia spends scan-converting them.

So: leave the prefs alone, take the software numbers as the ones that ship, and treat a demotion as the browser picking the faster backend rather than as a regression. **Any tuning decision measured against the accelerated backend is measured against the wrong one** — the pressures invert, since software rewards fewer, larger fills and does not care about tessellation bytes at all.

**Diagnostics.** `gfx.canvas.accelerated.debug` paints a 16x16 green square at the top-right of every _accelerated_ canvas each frame; a software canvas never gets one, so it reads demotion directly and says which canvas demoted. The layer canvases are never in the DOM, so getting at it needs a handle on the viewport — expose one temporarily from the example — then `getImageData(w - 8, 4, 1, 1)` on `viewport.layers` / `.shadeLayers` / `.fogLayers` and test for exact `(0, 255, 0)`. On demand, never per frame: `getImageData` is itself a readback and counts in the ratio above. Note the layers composite at the same origin, so their squares stack in the final image and only a per-canvas read tells them apart.

The points actually pushed to filled paths are counted per pass and shown as **Vertices** in the debug panel's Fill/Shade/Fog groups (`STATS_VERTICES_OFFSET` past each pass's draw-call slot in `statsBuffer`) - that is the number to watch here, not draw calls. One more counter is worth re-adding to `weldFlushSlot` whenever this is measured again: the ring vertices surviving collinear decimation. Vertices is that plus two per edge that steps out, so their ratio is 1.0 with seam repair off and 3.0 when every boundary edge is owned, and it is the only way to see expansion's contribution separately from the geometry's.

Relevant prefs and their defaults: `gfx.canvas.accelerated.profile-frames` 10, `profile-fallback-ratio` 0.3, `profile-cache-miss-ratio` 0.66, `gpu-path-size` 4 (MB), `gpu-path-complexity` 4000 path verbs, `cache-items` 8192 and `cache-size` 256MB (both texture handles, not paths). `MAX_POLY_VERTS` at 128 is two orders of magnitude inside the complexity limit, so path complexity has never been the binding constraint.
