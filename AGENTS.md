...
Two counters are worth re-adding to `weldFlushSlot` whenever this is measured again, since neither is derivable from the draw-call count: the ring vertices surviving collinear decimation, and the points actually pushed to the path, which is that plus two per edge that steps out. Their ratio is 1.0 with seam repair off and 3.0 when every boundary edge is owned, and it is the only way to see expansion's contribution separately from the geometry's.
